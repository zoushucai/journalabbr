...
#' @param iscorrect logical, if \code{TRUE} the  Fields in \code{tibble} will complete braces to make them appear in pairs.
#' @param connect_author character, what symbols are used to connect multiple authors, \code{'','\\\\&', '&', 'and'}, where \code{''} stand for do nothing(default).
#' @return \code{file} as a character string, invisibly.
#' @importFrom stringr str_count str_dup str_to_title
